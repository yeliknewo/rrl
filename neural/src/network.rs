...
impl<W: Float + SampleRange + FromPrimitive> NeuralNetwork<W> {
    ...
    pub fn new_random<S: Num + PartialOrd + Clone + ToPrimitive>(first_input_size: S, layer_sizes: &Vec<S>, min_weight: W, max_weight: W, min_bias: W, max_bias: W) -> NeuralNetwork<W> {
        let mut layers = vec![];

        let mut input_size = first_input_size;

        for layer_size in layer_sizes {
            layers.push(NeuralLayer::new_random(input_size, layer_size.clone(), min_weight, max_weight, min_bias, max_bias));
...
impl<W: SampleRange + Float + FromPrimitive> NeuralLayer<W> {
    ...
    fn new_random<S: Num + PartialOrd + Clone + ToPrimitive>(num_inputs: S, num_outputs: S, min_weight: W, max_weight: W, min_bias: W, max_bias: W) -> NeuralLayer<W> {
        let mut neurons = vec![];
...
impl<W: Float + SampleRange + FromPrimitive> Neuron<W> {
    ...
    fn new_random<S: Num + PartialOrd + Clone + ToPrimitive>(num_inputs: S, min_weight: W, max_weight: W, min_bias: W, max_bias: W) -> Neuron<W> {
        let mut weights = vec![];
