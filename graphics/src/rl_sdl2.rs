...
pub fn build_window(window_settings: WindowSettings) -> GfxWindow<Window, Extras> {
    let sdl = sdl2::init().unwrap_or_else(|err| {
        panic!("Error while sdl2::init: {:?}",
               err)
    });

    let video = sdl.video()
        .unwrap_or_else(|err| {
            panic!("Error while making sdl.video(): {:?}",
                   err)
        });

    let mut event_pump = sdl.event_pump()
        .unwrap_or_else(|err| {
            panic!("Error while making event pump: {:?}",
                   err)
        });
    ...
    let controller_subsystem = sdl.game_controller()
        .unwrap_or_else(|err| {
            panic!("Error while making controller sub system: {:?}",
                   err)
        });
    controller_subsystem.set_event_state(true);

    let gl_attr = video.gl_attr();
    gl_attr.set_context_version(3,
                                2);
    gl_attr.set_context_profile(sdl2::video::GLProfile::Core);

    let (title, width, height) = window_settings;

    let mut builder = video.window(title,
                                   width,
                                   height);
