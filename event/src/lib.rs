...
pub fn two_way_channel<T, F>() -> TwoWayChannel<T, F> {
    ...
    (FrontChannel::new(send_to,
                       recv_from),
     BackChannel::new(send_from,
                      recv_to))
}
...
impl<T, F> FrontChannel<T, F> {
    ...
    pub fn send_to(&mut self,
                   event: T) {
        self.send_to.send(event).unwrap_or_else(|err| {
            panic!("failed to send to because: {}",
                   err)
        });
    }

    pub fn recv_from(&mut self) -> F {
        self.recv_from.recv().unwrap_or_else(|err| {
            panic!("recv from error: {}",
                   err)
        })
    }
...
impl<T, F> BackChannel<T, F> {
    ...
    pub fn send_from(&mut self,
        ...
        self.send_from
            .send(event)
            .unwrap_or_else(|err| {
                panic!("failed to send from because: {}",
                       err)
            });
    }

    pub fn recv_to(&mut self) -> T {
        self.recv_to.recv().unwrap_or_else(|err| {
            panic!("recv to error: {}",
                   err)
        })
    }
